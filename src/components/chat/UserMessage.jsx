...
export default function UserMessage({msg}) {
  ...
  return (
      <div className="d-flex align-items-center text-right justify-content-end ">
          <div className="pr-2"> <span className="name">{msg?.sender?.username}</span>
              <p className="msg">{msg?.content}</p>
              <span className='name'>{formattedDate}</span>
          </div>
          <div><img src="https://cdn.pixabay.com/photo/2020/07/01/12/58/icon-5359553_1280.png" width="30" className="img1" /></div>
      </div>
  )
}
